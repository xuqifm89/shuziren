<template>
  <div class="crud-container">
            ...
            <template #default="scope">
              ...
              <div v-else-if="column.type === 'preview'" class="preview">
                ...
                <div
                  ...
                  @click="playPreviewVideo(scope.row, column.prop)"
                >
                  <img
                    v-if="scope.row.coverPath"
                    :src="getFullUrl(scope.row.coverPath)"
                    class="preview-video-thumb"
                    loading="lazy"
                  />
                  <div v-else class="preview-video-icon">
                    <span>▶</span>
                  </div>
                </div>
                <audio
    ...
    <div v-if="videoPreviewVisible" class="custom-modal-overlay" @click.self="closeVideoPreview">
      <div class="custom-modal" style="width: 700px;">
        <div class="custom-modal-header">
          <h3>视频预览</h3>
          <button class="close-btn" @click="closeVideoPreview">×</button>
        </div>
        <div class="custom-modal-body" style="display:flex;justify-content:center;align-items:center;min-height:300px;">
          <video
...
const uploadUrl = computed(() => `${base.value}/api/${props.apiPath}/upload`)
...
const fetchData = async () => {
  ...
  try {
    ...
    let url = `${base.value}/api/${props.apiPath}?page=${currentPage.value}&size=${pageSize.value}`

    if (userId && !props.showAll) {
      url += `&userId=${userId}`
    }
...
const handleSubmit = async () => {
  try {
    ...
    const url = isEditing.value
      ? `${base.value}/api/${props.apiPath}/${formData.value.id}`
      : `${base.value}/api/${props.apiPath}`
...
const handleFileUpload = async (event, prop) => {
  ...
  try {
    const response = await fetch(`${base.value}/api/${props.apiPath}/upload`, {
      method: 'POST',
      body: uploadFormData
    })
...
const playPreviewVideo = (row, prop) => {
  const videoUrl = getFullUrl(row[prop] || row.videoPath || row.fileUrl)
  if (videoUrl) {
...
.preview-video-placeholder:hover {
