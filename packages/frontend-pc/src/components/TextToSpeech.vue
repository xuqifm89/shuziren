...
const playSelectedVoice = () => {
  ...
  // 构建音频URL
  const audioUrl = `${selectedVoice.value.fileUrl}`
...
const handleVoiceFileSelect = async (event) => {
  ...
  try {
    ...
    const uploadResponse = await fetch('/api/voice-library/upload', {
      method: 'POST',
    ...
    const saveResponse = await fetch('/api/voice-library', {
      method: 'POST',
