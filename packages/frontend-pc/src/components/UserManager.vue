...
function authFetch(url, options = {}) {
  options.headers = { ...options.headers, ...getAuthHeaders(options.headers || {}) }
...
async function handleSubmitUser() {
  ...
  try {
    ...
    if (isEditing.value) {
      // 更新用户
      response = await authFetch(`${API_BASE}/${userForm.id}`, {
        method: 'PUT',
      ...
      // 创建用户
      response = await authFetch(`${API_BASE}/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function handleSubmitPassword() {
  ...
  try {
    const response = await authFetch(`${API_BASE}/${currentUser.value.id}/password`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function handleDeleteUser(user) {
  try {
    const response = await authFetch(`${API_BASE}/${user.id}`, {
      method: 'DELETE'
    })
...
async function handleBatchDelete() {
  try {
    ...
    for (const user of selectedUsers.value) {
      try {
        const response = await authFetch(`${API_BASE}/${user.id}`, { method: 'DELETE' })
