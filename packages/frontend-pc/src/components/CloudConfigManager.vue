...
function authFetch(url, options = {}) {
  const isLocalApi = url.startsWith('/api/') || url.startsWith(API_BASE)
  if (isLocalApi) {
...
async function handleSaveAll() {
  ...
  try {
    ...
    for (const config of configData) {
      try {
        // 查找是否已存在
        const listResponse = await authFetch(`${API_BASE}?configKey=${config.configKey}`)
        ...
        if (listData.success && listData.data.length > 0) {
          ...
          saveResponse = await authFetch(`${API_BASE}/${existingId}`, {
            method: 'PUT',
          ...
          // 创建新配置
          saveResponse = await authFetch(API_BASE, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(config)
          })
  ...
  try {
    ...
    if (!configs.siliconflow.apiKey) {
      ...
      try {
        ...
        const response = await authFetch(`${sfUrl}/models`, {
          method: 'GET',
          headers: {
