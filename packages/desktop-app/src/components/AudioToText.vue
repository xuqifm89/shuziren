...
const loadCopyLibrary = async () => {
  ...
  try {
    let url = 'http://localhost:3001/api/copy-library'
...
const extractCopywriting = async () => {
  ...
  taskManager.executeAfterConfirm(async () => {
    ...
    try {
      ...
      const response = await authFetch('http://localhost:3001/api/text/extract-from-video', {
        method: 'POST',
...
const generateCreative = async () => {
  ...
  taskManager.executeAfterConfirm(async () => {
    try {
      ...
      const response = await authFetch('http://localhost:3001/api/text/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: userPrompt,
...
const optimizeCopywriting = async () => {
  ...
  try {
    const response = await authFetch('http://localhost:3001/api/text/rewrite', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
const addToCopyLibrary = async () => {
  ...
  try {
    const user = getCurrentUser()
    const response = await authFetch('http://localhost:3001/api/copy-library', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: '用户文案',
