...
const progressHint = ref('正在连接服务器...')
...
const startProgressAnimation = () => {
  ...
  progressInterval = setInterval(() => {
    if (progressPercent.value < 90) {
      progressPercent.value += Math.random() * 3
      if (progressPercent.value > 90) progressPercent.value = 90

      if (Math.random() > 0.7) {
        hintIndex = (hintIndex + 1) % hints.length
        progressHint.value = hints[hintIndex]
      }
    }
  }, 800)
}
...
const handleCancel = () => {
  console.log('🛑 [TaskProgressDialog] 用户点击取消按钮, 当前状态:', props.status)
