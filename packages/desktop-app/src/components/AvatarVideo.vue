            ...
            >
              <div class="avatar-option">
                <div class="avatar-preview-thumb">
                  <img :src="`http://localhost:3001${image.fileUrl}`" alt="" />
                </div>
        ...
        <div class="avatar-select-row">
          ...
          >
            ...
            >
              <div class="avatar-option">
                <div class="avatar-preview-thumb">
                  <img v-if="avatar.type === 'image'" :src="`http://localhost:3001${avatar.fileUrl}`" alt="" />
                  <video v-else :src="`http://localhost:3001${avatar.fileUrl}`" muted></video>
                </div>
    ...
    <div v-if="showPreviewModal" class="preview-modal-overlay" @click="closePreviewModal">
      <div class="preview-modal-content" @click.stop>
        ...
        <template v-if="previewFileType === 'video'">
          <video :src="previewFileUrl" controls class="preview-media"></video>
        </template>
        <template v-else>
          <img :src="previewFileUrl" alt="预览图片" class="preview-media" />
        </template>
      </div>
    </div>
  </div>
...
const fetchLatestVideo = async () => {
  try {
    ...
    let url = 'http://localhost:3001/api/work-library'
    if (user?.id) {
      url += `?userId=${user.id}`
    }
    ...
    if (Array.isArray(data) && data.length > 0 && data[0].videoPath) {
      videoPath.value = 'http://localhost:3001' + data[0].videoPath
    }
  } catch (err) {
...
const fetchImages = async () => {
  ...
  try {
    const user = getCurrentUser()
    let url = 'http://localhost:3001/api/portrait-library?type=image'
    if (user && user.id) {
      url += '&userId=' + user.id
    }
...
const previewAudio = () => {
  ...
  if (selectedAudio.value) {
    const audioUrl = 'http://localhost:3001' + selectedAudio.value.fileUrl
...
const handleImageFileSelect = async (event) => {
  ...
  try {
    ...
    const uploadResponse = await authFetch('http://localhost:3001/api/portrait-library/upload', {
      method: 'POST',
      body: formData
    })
    ...
    const saveResponse = await authFetch('http://localhost:3001/api/portrait-library', {
      method: 'POST',
...
const generateImageToVideo = async () => {
  ...
  taskManager.executeAfterConfirm(async () => {
    try {
      ...
      const imageFileUrl = 'http://localhost:3001' + selectedImage.value.fileUrl
      const audioFileUrl = 'http://localhost:3001' + selectedAudio.value.fileUrl
      const user = getCurrentUser()
      ...
      const response = await authFetch('http://localhost:3001/api/audio/generate-image-to-video', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
const fetchAvatars = async () => {
  ...
  try {
    const user = getCurrentUser()
    let url = 'http://localhost:3001/api/portrait-library'
    if (user && user.id) {
      url += '?userId=' + user.id
    }
...
const previewSound = () => {
  ...
  if (selectedSound.value) {
    const audioUrl = 'http://localhost:3001' + selectedSound.value.fileUrl
    console.log('🔊 播放音频:', audioUrl)
...
const previewAvatar = () => {
  ...
  if (selectedAvatar.value) {
    previewFileUrl.value = 'http://localhost:3001' + selectedAvatar.value.fileUrl
...
const previewImage = () => {
  ...
  if (selectedImage.value) {
    previewFileUrl.value = 'http://localhost:3001' + selectedImage.value.fileUrl
...
const handleDubbingFileSelect = async (event) => {
  ...
  try {
    ...
    const uploadResponse = await authFetch('http://localhost:3001/api/dubbing-library/upload', {
      method: 'POST',
      body: formData
    })
    ...
    const saveResponse = await authFetch('http://localhost:3001/api/dubbing-library', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const generateDubbing = async () => {
  ...
  try {
    const response = await authFetch('http://localhost:3001/api/audio/generate-dubbing', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        voiceFileUrl: 'http://localhost:3001' + selectedSound.value.fileUrl,
...
const generateVideo = async () => {
  ...
  if (selectedSound.value && selectedSound.value.fileUrl && !generatedDubbingUrl.value) {
    audioToUse = 'http://localhost:3001' + selectedSound.value.fileUrl
  }
  ...
  taskManager.executeAfterConfirm(async () => {
    try {
      ...
      if (props.useVideoDriver) {
        const videoFileUrl = 'http://localhost:3001' + selectedAvatar.value.fileUrl

        let data
        const response = await authFetch('http://localhost:3001/api/audio/generate-video-to-video', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        const response = await authFetch('http://localhost:3001/api/video/generate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        console.log('🎬 生成结果:', data)
        if (data.videoPath) {
          videoPath.value = 'http://localhost:3001/' + data.videoPath
