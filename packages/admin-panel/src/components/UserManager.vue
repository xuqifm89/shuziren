...
async function handleSubmitUser() {
  ...
  try {
    ...
    ElMessage.error(error.error || error.message || (isEditing.value ? '更新失败' : '创建失败'))
  } finally {
...
async function handleSubmitPassword() {
  ...
  try {
    await api.put(`/users/${currentUser.value.id}/password`, {
      oldPassword: '',
    ...
    ElMessage.error(error.error || error.message || '密码重置失败')
  } finally {
