...
async function loadConfigs() {
  try {
    const data = await api.get('/cloud-config')
...
async function handleSaveAll() {
  ...
  try {
    ...
    for (const config of configData) {
      try {
        const listData = await api.get('/cloud-config', { params: { configKey: config.configKey } })
        
        if (listData.success && listData.data && listData.data.length > 0) {
          const existingId = listData.data[0].id
          await api.put(`/cloud-config/${existingId}`, config)
        } else {
          await api.post('/cloud-config', config)
        }
        successCount++
      } catch (error) {
        const errMsg = error?.response?.data?.error || error?.message || '未知错误'
        errorMessages.push(`${config.configKey}: ${errMsg}`)
        console.error(`保存配置 ${config.configKey} 失败:`, errMsg)
      }
    }
    ...
    if (successCount === configData.length) {
      ...
      ElMessage.warning(`部分配置保存成功 (${successCount}/${configData.length})`)
    } else {
      ElMessage.error(`保存失败: ${errorMessages[0] || '请检查网络连接和登录状态'}`)
    }
